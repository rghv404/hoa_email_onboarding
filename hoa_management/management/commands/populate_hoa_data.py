...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Creating {num_hoas} HOAs with an average of {avg_properties} properties each..."
        )
        ...
        for _i in range(num_hoas):
            ...
            num_properties = random.randint(
                max(1, avg_properties - 3), avg_properties + 3
            )

            for _j in range(num_properties):
                property_type = random.choice(
                    [choice[0] for choice in Property.PROPERTY_TYPES]
                )
                ...
                Property.objects.create(
                    ...
                    square_footage=random.randint(800, 3500)
                    if random.choice([True, False])
                    else None,
                    year_built=random.randint(1980, 2023)
                    if random.choice([True, False])
                    else None,
                    monthly_hoa_fee=Decimal(str(random.randint(150, 600)))
                    if random.choice([True, False])
                    else None,
